...
# with open(f'{dropout}/knn_output_{n_epochs}_cpu_150_{dropout}.npz', 'wb') as f:
...
with open(os.path.join(f"{dropout}/Puck__{n_epochs}_cpu_knn_150.csv"), mode='w', newline='') as file:
...
hexmap(embeddings, f"{dropout}/umap_{connectivity}_{n_epochs}_cpu_cuknn" if mnn else f"{dropout}/umap_{n_epochs}_cpu_cuknn")
